...
namespace Svelto.ECS.Example.Engines.Player.Gun
{
    public class PlayerGunShootingEngine : INodesEngine, IQueryableNodeEngine, IStep<DamageInfo>
    {
        ...
        public PlayerGunShootingEngine(EnemyKilledObservable enemyKilledObservable, Sequencer damageSequence) //, Sequencer bulletSequence)
        {
        ...
        void Tick(float deltaSec)
        {
            ...
            if (Input.GetButton("Fire1") && playerGunComponent.timer >= _playerGunNode.gunComponent.timeBetweenBullets && Time.timeScale != 0
                && playerGunComponent.numberOfBulletsLeft.value > 0)
            {
                playerGunComponent.numberOfBulletsLeft.value -= 1;
                Shoot();
            }
        ...
        readonly Type[] _acceptedNodes = { typeof(GunNode), typeof(PlayerNode), typeof(HUDNode), typeof(PlayerTargetNode) };

        GunNode                 _playerGunNode;
        EnemyKilledObservable   _enemyKilledObservable;
        Sequencer               _enemyDamageSequence;
        Sequencer               _playerLooseBullet;
